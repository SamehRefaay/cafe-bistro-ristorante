...
const GalleryPage = () => {
	...
	const handleMenuBtnClick = (tag: string) => {
		setCurrentTag(tag);
		const dataForCurrentTag = galleryData.filter((item) => item.tag.includes(tag));
			...
			<div className=' py-20 px-4 lg:px-0 bg-white'>
				<div className='container mx-auto'>
						...
						{menu.map((item, index) => (
							<button
								...
								className={`py-1 px-2 lg:px-5 font-poppins rounded-lg uppercase ${
									currentTag === item.tag ? 'bg-black text-[#eee]' : 'bg-white text-[#222]'
								}`}
								onClick={() => handleMenuBtnClick(item.tag)}
							>
								{index === 0 || index === menu.length - 1 ? (
									<>
										<span className='inline md:hidden'>{item.title.substring(0, 3)}</span>
					...
					<div className='mt-20 w-full grid gap-5 lg:gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
						{data.map((item) => (
							<div key={item.id} className='relative  overflow-hidden group rounded-lg'>
								<Image
